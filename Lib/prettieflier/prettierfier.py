...
fork from https://pypi.org/project/prettierfier/1.0.3/

prettify_xml() can be used for any XML text.

prettify_fb2() can be used for any FB2 XML text.
...
def prettify_fb2(xml_string, indent=1, debug=False):
    ...
    # ugly_xml = doc.toprettyxml(indent=indent_str, encoding='utf-8').decode()
    ...
    inline_space = RegExSub(r'[\n\s]*(</(strong|a|emphasis)>)([a-zA-Zа-яґіїєА-ЯҐІЇЄ0-9])', re.S,
                            r'\g<1> \g<3>')
    ...
    encoding = RegExSub(r'(<\?xml.+?)\?>', 0, r'\g<1> encoding="utf-8"?>')

    regexps = [inline_all_tags, inline_start, inline_a_start, inline_end, inline_space,
               nested_spaces_start, nested_spaces_end,
               whitespace_re, empty_tags, blankline_re, quot, encoding]
...
def prettify_html(html_string, debug=False):
    ...
    sup_end_space = RegExSub(r'[\n\s]*(</sup>)([a-zA-Zа-яґіїєА-ЯҐІЇЄ0-9])', re.S,
                             r'\g<1> \g<2>')
    ...
    # Adds a space between the ending inline tags and following words
    inline_space = RegExSub(r'[\n\s]*(</(strong|a|span)>)([a-zA-Zа-яґіїєА-ЯҐІЇЄ0-9])', re.S,
                            r'\g<1> \g<3>')

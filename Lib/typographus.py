...
class Typographus:
    ...
    def removeRedundantBlocks(self, str):
        ...
        str = re.compile("</?.*?/?>", re.U).sub(replace, str);
    ...
    def typo_text(self, str):
        ...
        rules_main = [
            ...
            {"pat": u'(^|[^a-zA-Zа-яА-Я])(%s)\.?\s?([А-Я0-9]+)' % shortages, "rep": u'\g<1>\g<2>.%s\g<2>' % sym['nbsp'],
             "mod": re.S},
            ...
            {"pat": u'([А-ЯA-Z]\.)\s?([А-ЯA-Z]\.)\s?([А-Яа-яA-Za-z]+)', "rep": u'\g<1>\g<2>%s\g<3>' % sym['nbsp'],
             "mod": re.S},
